...
interface NewCollectionSliderProps { }
...
const NewCollectionSlider :React.FC<NewCollectionSliderProps> = () => {

    const [collectionData, updateCollectionData] = useState<CollectionData[]>([])

    useEffect(() => {
        const url = "https://api.artemismarket.io/new-collections?_sort=order_seq:asc&_limit=8";
        fetch(url)
            .then((response) => response.json())
            .then((json) => {
                console.log('json', json)
                console.log('trending name: ', json[0].new_collection.name)
                                ...
                                return (
                                    <SwiperSlide>
                                        <IonCard className='collection-Card'>
                                            <img src={item.new_collection.banner_image.url} className='collection-banner'></img>
                                            ...
                                            <IonCardHeader>
                                                <div className='collectionlogo-container'>
                                                    ...
                                                    <img className='collectionlogo-icon' src={item.new_collection.logo_image.url}></img>
                                                </div>
                                                {/* <IonCardSubtitle>Card Subtitle</IonCardSubtitle> */}
                                                <IonCardTitle className='collection-title'>{item.new_collection.name}</IonCardTitle>
                                            </IonCardHeader>

                                            <IonCardContent className='collection-desc'>
                                                {item.new_collection.description.slice(0, 97)}...
                                            </IonCardContent>
                                        </IonCard>
                                    </SwiperSlide>
                                )
